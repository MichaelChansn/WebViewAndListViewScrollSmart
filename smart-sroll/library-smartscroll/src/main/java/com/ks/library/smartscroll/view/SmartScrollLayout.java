...
import com.ks.library.smartscroll.R;
...
public class SmartScrollLayout extends ViewGroup {
    ...
    public SmartScrollLayout(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * @param list
     */
    private void stopListViewFling(ScrollListView list) {
    ...
    protected void onFinishInflate() {
        ...
        mWebView = (ScrollWebView) getChildAt(0);
        mListView = (ScrollListView) getChildAt(1);
...
//            int minY = mCurrentPos + getListViewContentHeight();
     ...
     */
    private class WVHolder {
        long time;  // millisSecs
        int curTop; // 当前scroll的top
        
        private WVHolder(long time, int curTop) {
            this.time = time;
            this.curTop = curTop;
        }
    }
